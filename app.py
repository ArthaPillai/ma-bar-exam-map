...
import leafmap.foliumap as leafmap
import branca.colormap as cm
import json  # Add this import for loading local GeoJSON
...
# Sidebar – View selector
# ---------------------------------------------------------
view_mode = st.sidebar.radio("Map view", options=["State-wide", "Greater Boston (MBTA subway)"], index=0)
...
st.title(f"Massachusetts Bar Examinee Distribution Map – {title_suffix}")
st.markdown(f"**Current view:** *{view_mode}*  |  **Data:** *{title_suffix}*  \nHover over any area to view details.")
...
def build_map(agg_df: pd.DataFrame, geojson: dict, mbta_mode: bool) -> leafmap.Map:
    ...
    zoom = 11 if mbta_mode else 8
    m = leafmap.Map(center=center, zoom=zoom, locate_control=False, draw_control=False, measure_control=False)
    ...
    m.add_geojson(
        geojson, style_function=style_function, info_mode="on_hover",
        fields=["ZIP Code", "Area", "Sub_Area", "Examinees"],
        aliases=["ZIP Code", "Area", "Sub-area", "Examinees"],
    )

    # ---------------------------------------------------------
    # ADD MBTA SUBWAY LINES (only in Greater Boston mode)
    # ---------------------------------------------------------
    if mbta_mode:
        # Line colors (official MBTA branding)
        line_colors = {"RED": "#DA291C", "ORANGE": "#ED8B00", "BLUE": "#003DA5", "GREEN": "#00843D", "SILVER": "#8D8D8D"}

        # Load local ARC GeoJSON
        try:
            with open("GISDATA.MBTA_ARC.json", "r") as f:
                lines_geojson = json.load(f)

            for feature in lines_geojson["features"]:
                line_type = feature["properties"].get("line", "").upper()
                color = line_colors.get(line_type)
                if color:
                    m.add_geojson(
                        feature,
                        style={"color": color, "weight": 4, "opacity": 0.9},
                        layer_name=line_type,
                    )
