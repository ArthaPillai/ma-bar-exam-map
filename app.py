...
# Title
# ------------------------------------------
st.title("Massachusetts Transit and Roads Map")
...
map_choice = st.radio(
    "Select map to view:",
    ["Statewide", "Greater Boston (MBTA Subway)", "Greater Boston (Highways)"]
)
...
# Option 1 – Statewide
# ------------------------------------------
if map_choice == "Statewide":
    folium.TileLayer("cartodb positron").add_to(m)

# ------------------------------------------
# Option 2 – Greater Boston (MBTA Subway)
# ------------------------------------------
elif map_choice == "Greater Boston (MBTA Subway)":
    ...
    GeoJson(
        ...
        tooltip=folium.features.GeoJsonTooltip(fields=["route_id", "route_long_name"]),
    ).add_to(m)
...
# ------------------------------------------
# Option 3 – Greater Boston (Highways)
# ------------------------------------------
elif map_choice == "Greater Boston (Highways)":
    roads = gpd.read_file("ma_major_roads.geojson")

    # Filter only major (primary) roads
    major_roads = roads[roads["FEATURE_TY"].isin(["Primary Road"])]
    ...
    GeoJson(
        major_roads,
        name="Major Highways",
        style_function=lambda x: {"color": "orange", "weight": 2.5, "opacity": 0.8},
        tooltip=folium.features.GeoJsonTooltip(fields=["FULLNAME", "FEATURE_TY"]),
    ).add_to(m)
